...
<template>
  <div class="flex-grow flex flex-col items-center justify-center space-y-6 p-12">
    ...
    <div v-if="devices.length > 0" class="w-full max-w-sm space-y-2">
      <div v-for="device in devices" :key="device.url"
    ...
    <Button v-if="devices.length === 0" :label="isScanning ? 'Scanning...' : 'Scan for Devices'"
            icon="pi pi-search" @click="emit('scan')" :loading="isScanning" class="p-button-lg px-8 rounded-xl" />
  </div>
</template>
