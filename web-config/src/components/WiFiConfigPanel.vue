...
<template>
  <div v-if="config" class="space-y-6">
    <div class="flex items-center justify-between mb-4">
      ...
      <div class="flex space-x-2">
        <Button
          icon="pi pi-plus"
          :label="t('common.add')"
          @click="config.wifi.push({ ssid: '', pass: '' })"
          severity="success"
          size="small"
        />
    ...
    <div class="space-y-4">
      <div
        v-for="(net, index) in config.wifi"
        :key="index"
      ...
      >
        ...
        <Button
          ...
          severity="danger"
          text
          @click="config.wifi.splice(index, 1)"
          class="mb-0.5 hover:bg-red-500/10"
          v-if="config.wifi.length > 1"
        />
        <div
          class="absolute -left-2 top-1/2 -translate-y-1/2 w-1 h-8 bg-blue-500/20 rounded-full group-hover:bg-blue-500 transition-colors"
        ></div>
      </div>

      <div
        v-if="config.wifi.length === 0"
        class="text-center py-8 bg-gray-900/20 rounded-2xl border-2 border-dashed border-gray-800"
      >
